...
void Scanner::Init() {
	...
	keywords.set(L"IF", 13);
	keywords.set(L"THEN", 14);
	keywords.set(L"ELSE", 15);
	keywords.set(L"WHILE", 16);
	keywords.set(L"DO", 17);
	keywords.set(L"BEGIN", 18);
	keywords.set(L"END", 19);
	keywords.set(L"BEGIN_VAR", 20);
	keywords.set(L"END_VAR", 21);
	keywords.set(L"Integer", 23);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 11:
			...
			else {t->kind = 11; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 12:
			case_12:
